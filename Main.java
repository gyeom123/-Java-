...
class 사람인력관리소 {
	// 6. "add사람"을 3번 할때마다 해당정보를 저장할 수 있는 지역변수를 생성
	사람 a사람0;
	사람 a사람1; 
	사람 a사람2;
	...
	void add사람(String 이름, int 나이) {
		...
		// 6 - 2 add사람의 정보를 사람변수 "a사람1 ~ 3"에 저장
		if (번호 == 1) {
			a사람0 = a사람; // a사람(new 사람)에 있는 정보를 하나하나넘기는게 아니 변수'번호'를 이용하여 a사람(지역변수)에 넘김
		} else if (번호 == 2) { // 지역변수를 이용하여 필요한 경우에 출력할 수 있다.
			a사람1 = a사람;
		} else if (번호 == 3) {
			a사람2 = a사람;
		}
	...
	사람 get사람(int 번호) {
		// 매개변수 '번호'를 이용하여 "a사람0 ~ 2"에 있는 정보를 리턴을 이용하여 넘긴다
		if (번호 == 1) {
			return a사람0;
		} else if (번호 == 2) {
			return a사람1;
		...
		return null;
		// ※리턴값이 있는 함수는 어느경우에도 리턴을 해야하기 때문에 번호가 없는 경우에도 리턴을 해야한다
	}
